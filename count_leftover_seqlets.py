...
The three key inputs:
  ...
  --window   : the -w value used in the real run (default 1000). CRITICAL --
               using the wrong window inflates the candidate count.
...
def count_candidate_seqlets(scores_h5, window, window_size, flank, target_fdr,
    ...
    discovery). It MUST match the -w used in the real run, or the candidate
    count will be wrong (using the full window inflates the count).
    ...
    # Crop to the central `window` bp -- MoDISco's -w. Without this, extraction
    # scans the full input window and over-counts candidates.
...
def main():
    ...
    ap.add_argument("--window", type=int, default=1000,
                    help="MoDISco -w value (central crop). MUST match the real "
                         "run; the full input window inflates the count.")
    ...
    n_cand, n_pos, n_neg = count_candidate_seqlets(
        args.scores, args.window, args.window_size, args.flank, args.target_fdr,
